...
export function useQueues() {
  ...
  const createQueue = useCallback(async (projectId, { name, priority, concurrency_limit }) => {
    await api('/queues', {
      method: 'POST',
      token,
  ...
  const toggleQueueStatus = useCallback(async (queue, projectId) => {
    const status = queue.status === 'active' ? 'paused' : 'active';
    await api(`/queues/${queue.id}/status`, { method: 'PATCH', token, body: { status } });
    await loadQueues(projectId);
    notify(`Queue ${status}`);
  }, [token, notify, loadQueues]);
