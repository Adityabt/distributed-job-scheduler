...
export default function App() {
  ...
  useEffect(() => {
    if (activeProject) loadQueues(activeProject.id);
  }, [activeProject, loadQueues]);
  useEffect(() => {
    if (activeQueue) loadJobs(activeQueue.id, statusFilter);
  }, [activeQueue, statusFilter, loadJobs]);

  useEffect(() => {
    if (!activeQueue) return;
    const interval = setInterval(() => {
      loadJobs(activeQueue.id, statusFilter);
    }, 3000);
  ...
  return (
    <div className="djs">
            ...
            {!activeQueue ? (
              <div
                style={{
                  ...
                  color: "#4A5A66",
                  fontSize: 13,
                  border: "1px dashed #1E2932",
                  borderRadius: 6,
                  textAlign: "center",
                  padding: 24,
                }}
              >
                <span style={{ color: "#7A8A96", fontSize: 14 }}>
                  No queue selected
                </span>
                <span style={{ maxWidth: 280 }}>
                  Pick an organization, then a project, then a queue from the
                  left — that's where jobs live and run. Don't have any yet? Use
                  the <b style={{ color: "#F5A623" }}>+</b> button on each panel
                  to create one.
                </span>
              </div>
            ) : (
              ...
              >
                <QueueDetailHeader
                  queue={
                    queues.find((q) => q.id === activeQueue.id) || activeQueue
                  }
                ...
                <div>
                  ...
                  <p style={{ fontSize: 11, color: "#4A5A66", marginTop: 8 }}>
                    Every job created in this queue moves left to right through
                    these stages as it's picked up and run. A job only reaches{" "}
                    <b style={{ color: "#3DDC84" }}>completed</b> once it
                    succeeds — otherwise it retries according to this queue's
                    policy, or lands in{" "}
                    <b style={{ color: "#EF4444" }}>dead letter</b> after too
                    many failures.
                  </p>
                </div>
                <StatusFilterBar
      ...
      {showNewProject && (
        <Modal
          title={`New project in ${activeOrg?.name}`}
        ...
        >
          <SimpleForm
            ...
            submitLabel="create"
            onSubmit={async (v) => {
              await createProject(activeOrg.id, v.name);
      ...
      {showNewQueue && (
        <Modal
          title={`New queue in ${activeProject?.name}`}
      ...
      {showNewJob && (
        <Modal
          title={`New job in ${activeQueue?.name}`}
      ...
      {jobDetail && (
        <Modal title="Job detail" onClose={closeJobDetail} wide>
          <JobDetailView
            ...
            onDelete={async (id) => {
              await deleteJob(id);
              if (activeQueue) await loadJobs(activeQueue.id, statusFilter);
            }}
          />
        </Modal>
      )}
